...
`sdkwork-assets` is a **unified asset platform repository** with shared Rust/TypeScript contracts, ingestion orchestration, and a PC browser client. It consumes `sdkwork-drive` for storage/uploader and owns the global assets catalog API via `sdkwork-assets`.
...
| `sdkwork-drive` consumer integration | Aligned | `@sdkwork/drive-app-sdk` (upload), `@sdkwork/assets-app-sdk` (catalog), `sdkwork-assets-ingestion-drive` |
...
- Global assets API: `sdkwork-assets` (`/app/v3/api/assets*`)
- Upload: `client.uploader.uploadAttachment()` or `sdkwork-assets-ingestion-drive`
- Catalog types: consume `AssetItem` from `@sdkwork/assets-app-sdk`; map via `@sdkwork/assets-core/asset-mapping`
