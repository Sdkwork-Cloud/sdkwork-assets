use sdkwork_routes_drive_app_api::composition_host::acl;
use sdkwork_routes_drive_app_api::composition_host::acl_sql;
use sdkwork_routes_drive_app_api::composition_host::app_context::DriveRequestContext;
use sdkwork_routes_drive_app_api::composition_host::dto::PageRequest;
use crate::dto::{
    ...
    MediaResourceResponse, UpdateAssetRequest, ASSET_NODE_SELECT_COLUMNS,
};
use sdkwork_routes_drive_app_api::composition_host::error::{
...
use sdkwork_routes_drive_app_api::composition_host::hashing::sha256_raw_hex_separated;
use sdkwork_routes_drive_app_api::composition_host::ids::next_drive_id;
use sdkwork_routes_drive_app_api::composition_host::mappers::map_node_row;
use sdkwork_routes_drive_app_api::composition_host::node_repository::{find_active_node, find_node};
use sdkwork_routes_drive_app_api::composition_host::response::{
    no_content, success_created_resource, success_list_page_simple, success_resource,
    DriveListHttpResponse,
};
use sdkwork_routes_drive_app_api::composition_host::state::AppState;
use sdkwork_routes_drive_app_api::composition_host::validators::{normalize_optional_text, require_non_empty_text, validate_page_size_i64};
...
#[derive(Debug, Clone)]
struct AssetNodeRow {
    node: sdkwork_routes_drive_app_api::composition_host::dto::DriveNodeResponse,
    created_at: String,
    updated_at: String,
}
...
pub async fn create_asset_collection(
    State(state): State<AppState>,
    Extension(ctx): Extension<DriveRequestContext>,
...
pub async fn add_asset_collection_item(
    State(state): State<AppState>,
    Extension(ctx): Extension<DriveRequestContext>,
...
pub async fn delete_asset_collection_item(
    State(state): State<AppState>,
    Extension(ctx): Extension<DriveRequestContext>,
...
pub async fn create_asset_relation(
    State(state): State<AppState>,
    Extension(ctx): Extension<DriveRequestContext>,
    Path(asset_id): Path<String>,
...
pub async fn legacy_asset_upload_route_gone() -> (StatusCode, Json<ProblemDetail>) {
    problem(
        StatusCode::GONE,
...
pub async fn asset_method_not_allowed() -> (StatusCode, Json<ProblemDetail>) {
    problem(
...
fn parse_asset_page_request(
    ...
    let limit = validate_page_size_i64(
        page_size,
        sdkwork_routes_drive_app_api::composition_host::constants::DEFAULT_LIST_PAGE_SIZE,
        1,
        sdkwork_routes_drive_app_api::composition_host::constants::MAX_LIST_PAGE_SIZE,
        "page_size",
    )?;
...
fn derive_asset_kind(node: &sdkwork_routes_drive_app_api::composition_host::dto::DriveNodeResponse) -> String {
...
fn build_resource_snapshot(node: &sdkwork_routes_drive_app_api::composition_host::dto::DriveNodeResponse) -> Option<MediaResourceResponse> {
    if node.node_type == "virtual_reference" {
        return None;
    }
...
async fn record_change(
    ...
    .await
    .map_err(sdkwork_routes_drive_app_api::composition_host::error::map_service_error)
}
...
) -> Result<sdkwork_routes_drive_app_api::composition_host::dto::DriveNodeResponse, (StatusCode, Json<ProblemDetail>)> {
...
async fn ensure_asset_catalog_anchor(
    ...
    if find_node(pool, tenant_id, &catalog_node_id).await.is_err() {
        ...
        if let Err(error) = insert_result {
            if !is_unique_constraint_error(&error) {
                return Err(sdkwork_routes_drive_app_api::composition_host::error::internal_problem(format!(
